sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	var that = this;
	return e.extend("com.new.prjt.znew_arm_prjt.controller.myAccess", {
		formatter: formatter,
		_data: {
			"date": new Date()
		},
		onInit: function () {
			...
			//////////////////////////////////////////////////////////////////////////////////////
			var that = this;
			oUserID = that.getOwnerComponent().getModel("navModel").getData().oResult.user_id;
		...
		handleUploadPress: function () {
			...
			var oFileUploader = sap.ui.getCore().byId(this.getView().byId("fileUploader").getId().split('--')[0] + "--fileUploader");
			var sUrl = "/sap/opu/odata/sap/ZGRAC_USER_ACCESS_MANAGE_SRV/NARUserInformationSet(USER_ID='2020000865')/AttachementsSet";
		...
		onOrgSubmitPress: function (oEvent) {
			var oModel1 = this.getView().getModel("grac");
			var oOrgSelectData = this.getView().byId("idTableOrg").getTable();
			var oFilterTcode = new sap.ui.model.Filter("TCODE", sap.ui.model.FilterOperator.EQ, "SE38");
			var oFilterOrg = new sap.ui.model.Filter("ORG_LEVEL", sap.ui.model.FilterOperator.EQ, "");
			var oFilterFrom = new sap.ui.model.Filter("FROM_VALUE", sap.ui.model.FilterOperator.EQ, "");
			var oFilterTo = new sap.ui.model.Filter("", sap.ui.model.FilterOperator.EQ, "");
			oModel1.read("/NARRoleInformationSet", {
				filters: [oFilterTcode, oFilterOrg, oFilterFrom, oFilterTo],
		...
		moveToTable2: function () {
			...
			var oTcodeTable = this.byId("idTcode");
			oTable1.getSelectedContexts()[0].getObject().FT = "SEL";
			oConn = oTable1.getSelectedContexts()[0].getObject().CONNECTOR;
		...
		moveToTable1: function () {
			var oTable1 = this.byId("table1");
			var oTable2 = this.byId("table2");
			oTable2.getSelectedContexts()[0].getObject().FT = "AVL";
			oTable1.getModel().refresh(true);
			oTable2.getModel().refresh(true);
		},
		...
		onDropTable2: function (oEvent) {
			...
			oFfModel.setProperty("Rank", iNewRank, oDraggedRowContext);
			oFfModel.refresh(true);
		},

		onValueHelpRequestTCode: function (evt) {
		...
		onSysInfoReferesh: function (event) {
			...
			oModelCom.read("/SystemSet", {
				...
				error: function (event) {
					sap.ui.core.BusyIndicator.hide();
					sap.m.MessageBox.warning('System Information not available,Kindly raise a SNow request');
					return;
				}
			});
		},
		...
		onDaysChange: function (oEvent) {
			var oValue = oEvent && oEvent.getSource().getSelectedKey() || " 10";
			var oModel1 = this.getView().getModel("grac");
			var filterme = [];
		...
		onUnloackUser: function (event) {
			var that = this;
			if (event.oSource.getIcon() === "sap-icon://private") {
				event.oSource.setState("Warning")
			}
			var oModelCom = this.getView().getModel('grac');
			var oViewTable = this.byId("myAccesSysTable");
			oViewTable.getModel().refresh(true);
		...
		onNextStep: function (e) {
			if (e.getSource().getText() === "Next:System/Landscape Information") {
				...
				var aElementID = ["idNewText", "fanem", "lname", "managerID", "IDMOb", "idemail", "cbBusinessProcess"];
				var iErrorCount = 0;
		...
		onRiskRuleSet: function (eve) {
			if (!this._oValueHelpDialogRule) {
				this._oValueHelpDialogRule = sap.ui.xmlfragment(this.getView().getId(), "com.new.prjt.znew_arm_prjt.fragment.RuleSetF4", this);
				this.getView().addDependent(this._oValueHelpDialogRule);
			}
			var oModel = this.getView().getModel("grac");
			var oF4Table = this.getView().byId("idRuleSetF4");
			oModel.read("/RulesetSet", {
				success: function (data) {
					var oAccessModel = new sap.ui.model.json.JSONModel({
						F4Values: data.results
					});
					oF4Table.setModel(oAccessModel);
				}.bind(this),
				error: function (error) { }
			});
			this._oValueHelpDialogRule.getBinding("items");
			var oBinding = this._oValueHelpDialogRule.getBinding("items");
			this._oValueHelpDialogRule.open();
		},
		onGetRiskAnalysis: function (e) {

			var oRuleSet = this.getView().byId("idRuleSet").getValue();
			if (oRuleSet === " ") {
				sap.m.MessageBox.error('Please enter a Ruleset Value');
			};
			var RoleInfo = [];
			roledata = {
				"ACTRULEID": oRuleSet
			...
			var RoleTableModel = this.getOwnerComponent().getModel("RoleInfoModel").getData().RolInfoData;
			if (RoleTableModel.length !== 0) {
				var roledata = {
					"USER_ID": oUserID
				};
				RoleInfo.push(roledata);
				RoleTableModel.forEach((item, index) => {
					var oRoleConn = RoleTableModel[index].System;
					roledata = {
						"CONNECTOR": oRoleConn
					};
					RoleInfo.push(roledata);
					roledata = {
						"ROLE_ID": RoleTableModel[index].role
					};
					RoleInfo.push(roledata);
				});
				var oPayload = {
					"USER_ACTION": "RISK",
					"RiskAnalysisSet": RoleInfo
				};
				var RiskTableModel = this.getOwnerComponent().getModel("RiskInfoModel");
				var oModel = this.getView().getModel('grac');
				var params = [];
				var GUID;
				params["X-CSRF-Token"] = oModel.getSecurityToken();
				...
				oModel.create('/NARUserInformationSet', oPayload, {
					...
					success: function (data) {
						GUID = data.GUID;
						//get Risk Analysis from GUID
						var oFilter = [];
						if (GUID) {
							oFilter.push(new sap.ui.model.Filter("GUID", sap.ui.model.FilterOperator.EQ, GUID));
							oModel.read("/RiskAnalysisSet", {
								async: true,
								filters: oFilter,
								urlParameters: {
									"$top": 10,
								...
								success: function (data1) {
									if (data1.results.length === 0) {
										...
										for (var i = 0; i < data1.results.length; i++) {
											var RiskData = {
												userid: data1.results[i].USER_ID,
												riskid: data1.results[i].RISKID,
												ruleid: data1.results[i].ACTRULEID,
												rulelevel: data1.results[i].DESCN,
												system: data1.results[i].CONNECTOR,
												action: data1.results[i].ACTION,
												key: i + 1
											};
											RiskTableModel.getData().RiskInfoData.push(RiskData);
											RiskTableModel.refresh(true);
										}
									}
								},
								error: function (event) {
									sap.m.MessageBox.error('No Data found for the GUID');
									return;
								}
							});
						}
				...
				sap.m.MessageBox.error("Role is not available for Risk Analysis");
			}
		},
		...
		onNextStepModify: function (e) {
			...
			if (onNext === "Next:System/Landscape Information") {
				var aElementID = ["idModifyText", "fanemMod", "lnameMod", "managerIDMod", "idDepartMod",
					"IDMObMod", "idemailMod",
				];
		...
		onUploadExcelRole: function (e) {
			var t = this;
			if (this.validateBeforeUploadRole() === false) {
				return
			}
			this._getDialogRole().close();
			var o = this.getView().byId("tableRole");
			var a = sap.ui.getCore().byId("idfileUploaderRole");
			var i = a.getValue();
			this.getView().byId("lblFileNameRole").setText("File: " + a.getFocusDomRef().files[0].name);
			...
			l.onload = function (e) {
				...
				function r() {
					...
					var d = t.getView().getModel("uploadRoleModel");
					for (var l = 0; l < n.length; l++) {
						d.getData().items.push(n[l])
					}
					d.refresh()
				}
				r()
		...
		onBeforeRendering: function () {
			...
			this.onUserValidationForFFAccessTab();
		},
		onUserValidationForFFAccessTab: function() {
			var oModel1 = this.getView().getModel("grac");
			var oUserID = this.getOwnerComponent().getModel("navModel").getData().oResult.user_id;
			...
			oModel1.read("/approval_inbox_accessSet", {
				...
				success: function (oData) {
					...
					if (oData.results[0].FF_Access === "X") {
						bFlag = true;
						that.getView().getModel("TilesData").setProperty("/access", true); //added by Prasanth on 30-11-2024
					}
					that.getView().byId("idFFAssignTab").setVisible(bFlag);
					that.getView().byId("newFireCReqSet").setVisible(bFlag);
					that.getView().byId("idFFObjStatus") && that.getView().byId("idFFObjStatus").setVisible(bFlag);
					// that.getView().byId("idHBox").getItems()[4].setVisible(bFlag);
					// that.getView().getModel("TilesData").setProperty("/access", true);
		...
		responsiveSystemButPress: function () {
			if (!this.dialog) {
				this.dialog = sap.ui.xmlfragment(this.getView().getId(), "com.new.prjt.znew_arm_prjt.view.f4_selection", this);
		...
		FFselectedIdDrop: function (oEvent) {
			var oDragSession = oEvent.getParameter("draggedControl");
			var pathTable1 = oDragSession.oBindingContexts.FireFiIdsel.sPath;
			var oModelTable2 = this.getOwnerComponent().getModel("FireFiIds");
			var oModelTable1 = this.getOwnerComponent().getModel("FireFiIdsel");
			var oData1 = [];
			oData1.push(oModelTable1.getProperty(pathTable1));
			if (oData1.length !== 0) {
				oModelTable2.getData().FireFiId.push(oModelTable1.getProperty(pathTable1));
		...
		onSubmitFF: function (Oevent) {
			var textFlag = Oevent.getSource().getText();
			if (textFlag === "Submit") {
				this._onSubmitFF("SUBMIT");
			} else if (textFlag === "Save") {
				this._onSubmitFF("SAVE");
			}
		},
		_onSubmitFF: function (saveOrSubmit) {
			var text = this.getView().byId("idFfidText").getValue();
			var oFlagSavSub = saveOrSubmit;
			var oModel = this.getView().getModel("grac");
			var that = this;
			that.flagM = oFlagSavSub;
			var oUserInfoModel = this.getOwnerComponent().getModel("userInfo1").getData();
			oUserInfoModel.EMPTYPE = this.getView().byId("idFfEmpType").getSelectedKey();
			var oSeleSysLandInfo = this.getOwnerComponent().getModel("SeleSysLandInfoFF").getData().sleSysLaData;
			var SysLandInfo = [];
			if (oSeleSysLandInfo.length !== 0) {
				oSeleSysLandInfo.forEach((item, index) => {
					var tdata = {
			...
			var finalTab = this.getView().byId("idFfidSelTable");
			var aData = (finalTab.getItems() || []).map(function (oItem) {
				// assuming that you are using the default model  
				return oItem.getBindingContext().getObject();
			});
			...
			if (aData.length !== 0) {
				aData.forEach((item, index) => {
					var lv_from = aData[index].VALID_FROM;
					var getFFdata = {
						"USER_ID": aData[index].USER_ID,
						"FFOBJECT": aData[index].FFOBJECT,
						"CONNECTOR": aData[index].CONNECTOR,
						"VALID_FROM": lv_from.replace(/-/gi, ''),
						"VALID_TO": aData[index].VALID_TO.replace(/-/gi, '')
					};
					FFVlidData.push(getFFdata);
				});
			}
		...
		thirdPartyUser: function (empType) {
			if (empType === "3rd Party User") {
				return true;
			} else {
				return false;
			}
		},
		thirdPartyUserTitle: function (empType) {
			if (empType === "3rd Party User") {
				return "3rd Party User Details";
			} else {
				return "";
			}
		},
		...
		_onMyAccessPress: function (oModel1) {
			if (sap.ui.getCore().byId("__xmlview4--idrootTitle")) {
				sap.ui.getCore().byId("__xmlview4--idrootTitle").setText("My Access");
			}
			...
			oModel1.read("/PersonnelSet", {
				...
				success: function (data, textStatus, jqXHR) {
					...
					oViewFirefighterSet.setModel(oFirefighterAccess);
					that.onNewFFRefreshBut(); //Added by Prasanth on 27-05-2024
			        that.onRemAccRefreshBut(); //Added by Prasanth on 27-05-2024
                    that.onShowAllReq(); //Added by Prasanth on 27-05-2024
					that.hideBusyIndicator();
		...
		OnApproversPopUp: function (Oevent) {
            var that = this;
            var filterme = []
            var oValue= Oevent.oSource.getParent().getBindingContext().getObject().EXTERNAL_KEY_DIS
            //var oValue = "'"+ oValue +"'"
            filterme.push(new sap.ui.model.Filter("EXTERNAL_KEY_DIS",sap.ui.model.FilterOperator.EQ, oValue));
            var oModel1 = this.getView().getModel("grac");
            oModel1.read("/NewUserRequestApproversSet", {
                filters: filterme,
                success: function (data, textStatus, jqXHR) {
                    var ReqPopUP = new sap.ui.model.json.JSONModel({
                        myItems: data.results
                    });
                    that.getView().setModel(ReqPopUP,"ReqPopUP"),
                    that.ApporverPopUpOpen()
                    sap.ui.core.BusyIndicator.hide();
                }.bind(this),
                error: function (evt) {
                    sap.ui.core.BusyIndicator.hide();
        ...
        ApporverPopUpOpen: function () {
            if (!this.ApporverPopUp) {
                this.ApporverPopUp = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.ApporverPopUp", this);
                this.getView().addDependent(this.ApporverPopUp);
            }
            // open dialog
            this.ApporverPopUp.setModel(this.getView().getModel("ReqPopUP"));
            this.ApporverPopUp.open();
        },
		OnRemApproversPopUp: function (Oevent) {
            var that = this;
            var filterme = []
            var oValue= Oevent.oSource.getParent().getBindingContext().getObject().EXTERNAL_KEY_DIS
            //var oValue = "'"+ oValue +"'"
            filterme.push(new sap.ui.model.Filter("EXTERNAL_KEY_DIS",sap.ui.model.FilterOperator.EQ, oValue));
            var oModel1 = this.getView().getModel("grac");
            oModel1.read("/RemoveAccessRequestApproversSet", {
                filters: filterme,
                success: function (data, textStatus, jqXHR) {
                    var ReqPopUP = new sap.ui.model.json.JSONModel({
                        myItems: data.results
        ...
        remApproverPopUpOpen: function () {
            if (!this.ApporverPopUp) {
                this.ApporverPopUp = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.remApproverPopUp", this);
                this.getView().addDependent(this.ApporverPopUp);
            }
            // open dialog
            this.ApporverPopUp.setModel(this.getView().getModel("ReqPopUP"));
		...
		OnFFApproversPopUp: function (Oevent) {
            var that = this;
            var filterme = []
            var oValue= Oevent.oSource.getParent().getBindingContext().getObject().EXTERNAL_KEY_DIS
            //var oValue = "'"+ oValue +"'"
            filterme.push(new sap.ui.model.Filter("EXTERNAL_KEY_DIS",sap.ui.model.FilterOperator.EQ, oValue));
            var oModel1 = this.getView().getModel("grac");
            oModel1.read("/FirefighterRequestApproversSet", {
                filters: filterme,
                success: function (data, textStatus, jqXHR) {
                    var ReqPopUP = new sap.ui.model.json.JSONModel({
                        myItems: data.results
        ...
        ffApproverPopUpOpen: function () {
            if (!this.ApporverPopUp) {
                this.ApporverPopUp = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.ffApproverPopUp", this);
                this.getView().addDependent(this.ApporverPopUp);
            }
            // open dialog
            this.ApporverPopUp.setModel(this.getView().getModel("ReqPopUP"));
		...
		onAfterRendering(){
			this.getView().byId("idAgeDays").setSelectedKey(" 10")
			this.onDaysChange();
			this.onRemAccRefreshBut();
			this.onNewFFRefreshBut();
			this.onSysInfoReferesh();
			this.onShowAllReq();

		},
		onNavToRequest: function (oEvent) {
			var that = this;
			var oRequest = oEvent.getSource().getText();
			if (!this._oRequestDetails) {
				this._oRequestDetails = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.RequestDetails", this);
				this.getView().addDependent(this._oRequestDetails);
			}
			var oRequestDetails = this.getView().getModel("requestDetails");
			//oRequestDetails.setUseBatch(false);
			oRequestDetails.read("/UserInformationSet", {
				filters:[new sap.ui.model.Filter("Request", "EQ", oRequest)],
				urlParameters: {
					"$expand": "UserToSystemNAV,UserToOrgTcodeNAV"
				},
				success: function (data) {
					var oUserDetailsModel = new JSONModel();
					oUserDetailsModel.setData(data.results[0]);
					that._oRequestDetails.setModel(oUserDetailsModel,"UserDetailsModel");
					that._oRequestDetails.open();
				},
				error: function (evt) {
		...
		onRequestInfoClose:function(){
			this._oRequestDetails.close();
		}
