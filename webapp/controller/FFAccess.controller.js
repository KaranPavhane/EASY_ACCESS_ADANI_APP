sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	var that = this;
	return e.extend("com.new.prjt.znew_arm_prjt.controller.FFAccess", {
		formatter: formatter,
		_data: {
			"date": new Date()
		},
		...
		someFunctionOfTheFirstController: function (sChannelId, sEventId, sData) {
			this.byId("idFFComboUserType").setSelectedKey("");
			this._clearFFRequest();
			this.onSuerSelectFF("Self");
		...
		onNextStepFireFighter: function (e) {
			if (e.getSource().getText() === 'Next:System/Landscape Information') {
				...
				var aElementID = ["idFfidText", "idFfFName", "idFfLName", "idFfRepMana",
					"idFfEmail",
				];
				var oElementID_SelectDrop = [ "idFfEmpType"];
				var iErrorCount = 0;
			...
			} else if (e.getSource().getText() === 'Next:Firefighter IDs') {
			...
			} else if (e.getSource().getText() === 'Next:Firefighter Validity') {

				var aSelectedFFContexts = this.getView().byId("DragTableFireFighterID").getSelectedContexts();
				if (aSelectedFFContexts.length === 0) {
					MessageBox.error("Please select atleast FF ID.");
					return false;
				}
				var aFFData = [];
			...
			} else if (e.getSource().getText() === "Next:Attachments") {

				this.byId("CreateProductWizardId261").nextStep();
				e.getSource().setVisible(false);
				this.byId("idNextReqSubAtc").setVisible(true);
				var AttDataRem = {
					"AttInfoData": []
				};
				this.getOwnerComponent().getModel("FFInfoModelMod").setData(AttDataRem);
			} else if (e.getSource().getText() === 'Next:Request Submission') {
				this.byId("CreateProductWizardId261").nextStep();
				this.byId("idFFButReqSub").setVisible(true);
				e.getSource().setVisible(false);
				this.getView().byId("idFfidSelTable").removeSelections();
				var oFfidTable = this.byId("idFfidSelTable");
				var oFfidSelectdata = this.getView().byId("idFFValidTable").getModel("FFVlid").getData().ffvData;
		...
		// 	var AttTableModel = this.getOwnerComponent().getModel("FFInfoModelMod");
		...
		// 			//var oFileUploader = sap.ui.getCore().byId("idfileUploaderFF");
		...
		// 				"AttInfoData": []
		// 			};
		// 			that.getOwnerComponent().getModel("FFInfoModelMod").setData(AttDataRem);
		...
		onFileSelect: function (oEvent) {
			...
			this.files = oEvent.getParameter("files");
			sap.ui.getCore().byId("idfileUploaderFF").setPlaceholder("Please Choose");
		},
		onAttachUpload: function (e) {

			this._oAttachDialog.close();
			var AttTableModel = this.getOwnerComponent().getModel("FFInfoModelMod");
			for (var oFiles = 0; oFiles <= this.files.length - 1; oFiles++) {
				...
				form_data.items.add(this.files[oFiles]);
				var AttData = {
					tile: this.files[oFiles].name,
					type: filetype,
					addedOn: new Date(),
					addedBy: oUserID,
					key: oFiles + 1,
					lastModified: this.files[oFiles].lastModified
				};
				AttTableModel.getData().AttInfoData.push(AttData);
			}
			AttTableModel.refresh(true);
			sap.ui.getCore().byId("idfileUploaderFF").setPlaceholder();
		},
		...
		openAttachDialog: function (e) {
			...
			sap.ui.getCore().byId("idfileUploaderFF").setButtonText("Please Choose");
			sap.ui.getCore().byId("idfileUploaderFF").setPlaceholder();
		},
		...
		getAttachDialog: function () {
			...
			if (!this._oAttachDialog) {
				this._oAttachDialog = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.AttachUploadDialogFF", this);
				this.getView().addDependent(this._oAttachDialog);
				sap.ui.getCore().byId("idfileUploaderFF").setPlaceholder();
			}
			var oPlace = "Please Choose";
			sap.ui.getCore().byId("idfileUploaderFF").setPlaceholder(oPlace);
			sap.ui.getCore().byId("idfileUploaderFF").setValue("");
		...
		deleteRowRole: function (e) {
			var t = e.getSource().getBindingContext("FFInfoModelMod").getObject();
			var oRoleDateFromTable = this.getOwnerComponent().getModel("FFInfoModelMod").getData().AttInfoData;
			for (var o = 0; o < oRoleDateFromTable.length; o++) {
				if (oRoleDateFromTable[o] == t) {
					oRoleDateFromTable.splice(o, 1);
					var a = this.getView().getModel("FFInfoModelMod");
					a.refresh();
					break
				}
			}
				...
				success: function (data) {
					...
					if (data) {
						...
						if (data.PHONE == "" || data.PHONE.length < 0) {
							that.getView().byId("idFfMNumber").setEditable(true);
						} else {
							that.getView().byId("idFfMNumber").setEditable(false);
						}
					}
				}.bind(this),
				error: function (evt) { }
		...
		FFuser: function (oEvent, user) {
			this._clearFFRequest();
			var oUser;
			var that = this;
			var oModel = this.getView().getModel("grac");
			if (oEvent) {
				oUser = oEvent.getSource().getSelectedKey();
			} else {
				oUser = user;
				this.byId("idComboUser2FF").setValue(user);
			}
			//oModel.read("/PersonnelSet(USER_ID='" + oUser + "',AD_ID='" + oUser + "')", {
			oModel.read("/PersonnelSet(AD_ID='" + oUser + "')", {
				success: function (data) {
					if (data) {
						var oUserInfoModel = that.getOwnerComponent().getModel("userInfo1");
						var data = {
							"DEPARTMENT": data.ZDEPTD,
							"EMAIL": data.EMAIL,
							"EMPTYPE": data.EMPTYPE,
						...
						oUserInfoModel.setData(data);
						oUserInfoModel.refresh(true);
						that.getView().byId("idNameFF").setText(data.FIRST_NAME + " " + data.LAST_NAME);
					}
				},
				error: function (data) {
					sap.m.MessageBox.error("User is not selected.");
					return;
				}
			});
		},
		...
		onOrgFromAuth: function (eve) {
			...
			var aFilters = [];
			if (!this._oValueHelpDialogFF) {
				this._oValueHelpDialogFF = sap.ui.xmlfragment(this.getView().getId(), "com.new.prjt.znew_arm_prjt.fragment.F4FromOrgVal", this);
				this.getView().addDependent(this._oValueHelpDialogFF);
			}
			...
			oModel.read("/SU53OrgValueFromSet", {
				...
				success: function (data) {
					...
					this._oValueHelpDialogFF.setModel(oAccessModel);
				}.bind(this),
				error: function (error) {
					sap.ui.core.BusyIndicator.hide();
				}
			});
			this._oValueHelpDialogFF.getBinding("items");
			var oBinding = this._oValueHelpDialogFF.getBinding("items");
			this._oValueHelpDialogFF.open();
		},
		...
		onF4Click: function (oTable, inum, oObject) {
			...
			var aFilters = [];
			if (!this._oValueHelpDialogFF) {
				this._oValueHelpDialogFF = sap.ui.xmlfragment("idOrgValF4", "com.new.prjt.znew_arm_prjt.fragment.F4FromOrgVal", this);
				this.getView().addDependent(this._oValueHelpDialogFF);
			}
			...
			oModel.read("/NARORGVALUEFROMSet", {
				...
				success: function (data) {
					...
					this._oValueHelpDialogFF.setModel(oOrgModel, "OrgModel");
					this._oValueHelpDialogFF.open();
				}.bind(this),
				error: function (error) {
					sap.ui.core.BusyIndicator.hide();
		...
		handleChangeToAndFrom: function (oEvent) {

			if (oEvent.oSource.getName() == "fromDate") {
				var fromDate = oEvent.oSource.getDateValue();
				oEvent.oSource.getParent().getCells()[3].setMinDate(fromDate);
				oEvent.oSource.getParent().getCells()[3].setValue("");
			}
			else if (oEvent.oSource.getName() == "toDate") {
				oEvent.oSource.setValueState("None");
				var fromdate = oEvent.oSource.getParent().getCells()[2].getDateValue();
				var todate = oEvent.oSource.getParent().getCells()[3].getDateValue();
				if (todate < fromdate) {
					oEvent.oSource.getParent().getCells()[3].setValue("");
					oEvent.oSource.setValueState("Error");
					oEvent.oSource.setValueStateText("To date Can not be less the from date");
				}
			}
