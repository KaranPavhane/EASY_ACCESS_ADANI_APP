sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	var that = this;
	return e.extend("com.new.prjt.znew_arm_prjt.controller.newModifyRequest", {
		formatter: formatter,
		_data: {
			"date": new Date()
		},
		...
		someFunctionOfTheFirstController: function (sChannelId, sEventId, sData) {
			...
			this.getView().byId("req_for").setSelectedKey("Self");
		...
		onNextStep: function (e) {
			if (e.getSource().getText() === "Next:System/Landscape Information") {
				...
				oModelCom.read("/NARSystemInformationSet", {
					...
					error: function (event) {
						sap.ui.core.BusyIndicator.hide();
						sap.m.MessageBox.error(JSON.parse(event.responseText).error.message.value);
						return;
					}
				});
			}
			...
			if (e.getSource().getText() === "Next:Attachments") {
				...
				var errors = [];
				var orgData = this.getOwnerComponent().getModel("OrgTableModel").getProperty("/OrgDataNormal");
				...
				for (var j = 0; j < orgData.length; j++) {
					if (
						...
						//orgItems[j].getCells()[3].getItems()[0].setEditable(true).setEnabled(true).setValueState("Error");
						errors.push([j])
					}
				}
				if (errors.length > 0) {
					...
					this.getOwnerComponent().getModel("AttInfoModelAdani").setData(AttDataRem);
		...
		_onSubmitOrSave: function (saveOrSubmit) {
			...
			//var oCSRFToken = oModel.getSecurityToken();
			var AttTableModel = this.getOwnerComponent().getModel("AttInfoModelAdani");
			...
			oModel.create('/NARUserInformationSet', oPayload, {
				...
				success: function (data) {
					...
					that.getOwnerComponent().getModel("AttInfoModelAdani").setData(AttDataRem);
		...
		onFileSelect: function (oEvent) {
			...
			sap.ui.getCore().byId("idfileUploader").setPlaceholder("Please Choose");
		},
		...
		onAttachUpload: function (e) {
			...
			sap.ui.getCore().byId("idfileUploader").setPlaceholder();
		},
		...
		openAttachDialog: function (e) {
			...
			sap.ui.getCore().byId("idfileUploader").setButtonText("Please Choose");
			sap.ui.getCore().byId("idfileUploader").setPlaceholder();
		},
		...
		getAttachDialog: function () {
			if (!this._oAttachDialog) {
				this._oAttachDialog = sap.ui.xmlfragment("com.new.prjt.znew_arm_prjt.fragment.AttachUploadAdani", this);
				this.getView().addDependent(this._oAttachDialog);
				sap.ui.getCore().byId("idfileUploader").setPlaceholder();
			}
			var oPlace = "Please Choose";
			sap.ui.getCore().byId("idfileUploader").setPlaceholder(oPlace);
		...
		deleteRowRole: function (e) {
			var t = e.getSource().getBindingContext("AttInfoModelAdani").getObject();
			var oRoleDateFromTable = this.getOwnerComponent().getModel("AttInfoModelAdani").getData().AttInfoData;
			for (var o = 0; o < oRoleDateFromTable.length; o++) {
				if (oRoleDateFromTable[o] == t) {
					oRoleDateFromTable.splice(o, 1);
					var a = this.getView().getModel("AttInfoModelAdani");
					a.refresh();
					break
				}
			}
		...
		_clearNewRequest: function () {
			var aElementID = ["idNewText", "fanem", "lname", "managerID", "idPNumber", "idDepart", "IDMOb", "idemail"];
			this.getView().byId("table1").removeSelections();
			this.getView().byId("table2").removeSelections();
			this.getView().byId("idTcode").removeSelections();
		...
		onDeleteTocde: function (e) {
			...
			this.getOwnerComponent().getModel("OrgTableModel").refresh(true)
		...
		// 	console.log("aData in Download : ",aData);
		...
		onF4Click: function (oTable, inum, oObject, eve) {
			...
			//var oF4Table = this.getView().byId("idF4");
			var aFilters = [];
			if (!this._oNewValueHelpDialog) {
				this._oNewValueHelpDialog = sap.ui.xmlfragment("idOrgValF4", "com.new.prjt.znew_arm_prjt.fragment.F4FromOrgVal", this);
				this.getView().addDependent(this._oNewValueHelpDialog);
			}
			...
			oModel.read("/NARORGVALUEFROMSet", {
				...
				success: function (data) {
					...
					this._oNewValueHelpDialog.setModel(oOrgModel, "OrgModel");
					this._oNewValueHelpDialog.open();
				}.bind(this),
				error: function (error) {
					sap.ui.core.BusyIndicator.hide();
		...
		// 		}
		// 		else if (e.getSource().getName() === "Tcode" && e.getSource().getName() && e.getSource().getValue() == "") {
				...
				success: function (oData) {
					...
					if (!this._oProfileVH) {
						this._oProfileVH = sap.ui.xmlfragment(
							"com.new.prjt.znew_arm_prjt.fragment.ProfileValueHelpNewModifyRequest",
							this
						);
				...
				success: function (oData) {
					...
					if (!that._oSubProfileVH) {
						that._oSubProfileVH = sap.ui.xmlfragment(
							"com.new.prjt.znew_arm_prjt.fragment.SubProfileVHNewModifyRequest",
							that
						);
		...
		onDesignationVH: function (oEvent) {
			...
			if (!this._oDesignationVH) {
				this._oDesignationVH = sap.ui.xmlfragment(
					"com.new.prjt.znew_arm_prjt.fragment.DesignationVHNewModifyRequest",
					this
				);
